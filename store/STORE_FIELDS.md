...
- JSONPath queries — filter with expressions like $.data[*].name, with match highlighting and navigation
- Request timing waterfall — mini color bar per request, detailed phase breakdown (blocked, DNS, connect, SSL, send, wait, receive)
- Pin/bookmark — pin important requests to the top, pinned items survive clear
...
- Export filtered requests as JSON or HAR 1.2 with real timing data
- JSON Lines (NDJSON) format support with auto-detection
- Chunked rendering for 10k+ node responses without lag

DEVELOPER UX

- Keyboard navigation: j/k to browse, / to search, n/N for matches, p for JSONPath, b to pin, ? for help
